...
export default function DrumLesson6() {
  ...
  const markLessonComplete = async () => {
    if (status === "authenticated" && session?.user?.email) {
      await fetch("/api/progress", {
        ...
        body: JSON.stringify({ userId: session.user.email, lessonId: "drums-6" }),
      });
    }
  };
  ...
  const handleSendForReview = async () => {
    ...
    try {
      ...
      formData.append("audio", audioBlob, "drum-performance.wav");
      formData.append("userId", session.user.email);
      formData.append("lessonId", "drums-6");
    ...
    <div className="bg-[#0d0d0d] text-slate-300 font-sans antialiased min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 pt-32 pb-24">
        <header className="mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-6">Lesson 6: Performance Groove Lab</h1>
          <p className="text-xl text-slate-400 leading-relaxed max-w-3xl">
            Bring everything together into a final performance routine: groove, transition, fill, control, and confidence.
          </p>
        </header>

        <section className="mb-12">
          <h2 className="text-2xl text-white mb-4">1. Warm-Up Plan</h2>
          <div className="bg-white/5 rounded-xl p-6 text-slate-400 space-y-2">
            <p>• 2 minutes single strokes</p>
            <p>• 2 minutes double strokes</p>
            <p>• 2 minutes basic 4/4 groove</p>
            <p>• 2 minutes fills and transitions</p>
          </div>
        </section>

        <section className="mb-12 p-8 rounded-2xl bg-white/5 border border-white/10">
          <h2 className="text-2xl text-white mb-4">2. Final Performance Challenge</h2>
          <p className="text-slate-400 mb-4">Play this full sequence without stopping:</p>
          <div className="bg-black/40 p-6 rounded-xl text-[#ff5a00] font-mono text-center space-y-2">
            <p>8 bars basic groove</p>
            <p>→ 1 short fill</p>
            <p>→ 8 bars groove with kick variation</p>
            <p>→ strong ending on beat 1</p>
          </div>
        </section>

        <section className="mb-12">
          <h2 className="text-2xl text-white mb-4">3. What to Listen For</h2>
          <ul className="list-disc list-inside text-slate-400 space-y-2">
            <li>Is your tempo steady from start to finish?</li>
            <li>Do your fills return cleanly to the groove?</li>
            <li>Are the snare backbeats clear and confident?</li>
            <li>Do you sound relaxed instead of tense?</li>
          </ul>
        </section>
        ...
        <section className="mb-16">
          ...
          <p className="text-slate-400 mb-4">
            Record your final groove performance and send it for tutor feedback.
          </p>
        ...
        <div className="flex justify-end">
          <Link href="/dashboard/student/drums" className="px-8 py-3 bg-[#ff5a00] text-white rounded-full hover:bg-white hover:text-black" onClick={markLessonComplete}>
            Finish Course
          </Link>
        </div>
      </main>
